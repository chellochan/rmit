...
if __name__ == '__main__':
    ...
    try:
        ...
        # changed to use w+ for create not exist output file
        output_file = open(output_filename, 'w+')
        print("{}-{}.out".format(data_filename, args[1]))
        analyse_file = open("{}-{}.out".format(data_filename, args[1]), 'w')
        for line in command_file:
            ...
            if command == 'S':
                ...
                analyse_file.write(
                    "Type: {type:>12}, Operation: {operation:>3}, Word: {word:15}, Time difference:{diff:.12f}\n".format(
                        type=args[1], operation=command, word=word, diff=time.time() - start))
