...
def find_job_ad_by_web_index(job_ad_list, web_index):
    for job_ad in job_ad_list:
        if job_ad.get_web_index() == web_index:
            return job_ad
    return None


def generate_word_vector_for_modelling(job_ad, model_ft):
    sum_words = 0
    num_words = len(job_ad.all_tokens)
    word_vec = []
    for word in job_ad.all_tokens:
        sum_words += model_ft.wv[word]
        word_vec.append(sum_words / num_words)
    return word_vec


def predict_category(lr_model, word_vec, labels, seed=0):
    y_pred = lr_model.predict(word_vec)
    return y_pred[0]


def get_latest_webIndex(job_ad_list):
    job_ad_list.sort(key=lambda job_ad: int(job_ad.get_web_index()), reverse=True)
    return int(job_ad_list[0].get_web_index()) + 1


def category_to_display(category):
